...
public class AccountAsyncTemplate
  ...
  @Override
  public ResultOrErrorFuture<AccountState> getState(final Account account) {
  ...
  @Override
  public ResultOrErrorFuture<TxHash> createName(final Account account, final String name,
      final long nonce) {
    return getCreateNameFunction().apply(account, name, nonce);
  }

  @Override
  public ResultOrErrorFuture<TxHash> updateName(final Account ownerAccount, final String name,
      final AccountAddress newOwner, final long nonce) {
    return getUpdateNameFunction().apply(ownerAccount, name, newOwner, nonce);
  }

  @Override
  public ResultOrErrorFuture<AccountAddress> getNameOwner(final String name) {
    return getNameOwnerFunction().apply(name);
  }
